...
if( $statusUpload == 0) {
    ...
    if(mysqli_num_rows($select) > 0){
        ...
        if(move_uploaded_file($_FILES['foto']['tmp_name'], $target_file)) {
            $query = mysqli_query($conn, "INSERT INTO tb_produk (foto,kode_produk,nama_produk,id_kategori,harga,stok,id_satuan ) values ('".$kode_rand.$_FILES['foto']['name']."','$kode_produk','$nama_produk','$id_kategori','$harga','$stok','$id_satuan')");
            if($query) {
